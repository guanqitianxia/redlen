...
String jud=request.getParameter("judge");if(jud!=null){if(jud.equals("1")){%><script>alert("打赏成功")</script><%}else{%><script>alert("失败，月票不足")</script><%}}
...
<div class="main_container">
 ...
 try
 {
	...
	while(rs.next()){
		...
		type=rs.getString("novelbase.type0");
		Llabel=rs.getString("novelbase.Llabel");
	}
 ...
 <div class="info_show">
    <div class="info_show_intro">
      ...
      if(!usertype.equals("2")){
    	  ...
    	  <ul>
            ...
            <li><button type="button" onclick="spare('<%=name%>')">月票打赏</button></li>
          </ul> 
    	  <%
      }
      %>
    ...
    <form action="TicSpareServlet?id=<%=id0%>&usertype=<%=usertype%>&nid=<%=nid%>" name="ticform" method="post">
    <input type="hidden" id="con" name="con"/>
    </form>
    <script>
    function spare(name){
    	var tic_num=prompt("请输入要打赏的月票数目");
    	if(!isNaN(tic_num)){
    		document.getElementById("con").value=tic_num+","+name;
    		document.ticform.submit();
    	}else{
    		alert("请输入数字");
    	}
    }
