...
{
	int chapternum=0,worknumber=0;
	try{
		Class.forName("com.mysql.cj.jdbc.Driver");
		conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/redlen?serverTimezone=UTC&&useUnicode=true&&characterEncoding=UTF-8","chenguanhao","jisuanji");
		Statement stmt=conn.createStatement();
		String sql="select * from novelmessage where nid="+nid;
		ResultSet rs=stmt.executeQuery(sql);
		while(rs.next())
		{
			chapternum=rs.getInt("chapternum");
			worknumber=rs.getInt("worknumber");
		}
		conn.close();
	}catch(Exception e){
			System.out.println("error"+e.toString());
...
<div class="catalog_write">
  ...
  <!-- 发布 -->
  <form action="NovelReleaseServlet?aid=<%=aid %>&nid=<%=nid %>" name="release" method="post">
  </form>
      
    <div class="catalog_header">
      <span>共<%out.print(chapternum); %>章<%out.print(worknumber); %>字</span>
      <button type="button" onclick="releas()">发布</button>
