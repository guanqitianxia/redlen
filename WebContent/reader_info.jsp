...
<div class="header">
  ...
  <a href="OutServlet?id=<%=username%>"><button style='float:right;font-size:15px;' id="cancel">注销</button></a>
</div>
<%
if(username==null)
{
...
if(username!=null)
{
	...
	{
		sql="select * from novelreaderinfo i,novelreaderwallet w,novelreadersubscribe c where i.rid=w.rid and w.rid=c.rid and w.rid="+rid;
		...
		//sql="select * from novelreaderwallet w where w.rid="+rid;
	}
...
<div class="catalog">
  <ul>
    ...
    <li><a href="reader_info.jsp?operate=sucribe&rid=<%=rid%>&usertype=<%=usertype%>">消费记录</a></li>
...
if(operate.equals("history")){
	%>
	<div class="history">
	 <table cellspacing="0">
      <caption>小说阅读时长记录</caption>
      <tr>
        <th>书籍编号</th>
        <th>书名</th>
        <th>作者</th>
	...
	<%
	String hsql="select * from novelbase b,readtime t where t.rid="+rid+" and t.nid=b.nid";
	rs=stmt.executeQuery(hsql);
	while(rs.next())
	{
		%>
		<tr>
		  <td><%=rs.getInt("t.nid") %></td>
		  <td><%=rs.getString("b.name") %></td>
		  <td><%=rs.getString("b.author") %></td>
		  <td><%=rs.getString("b.type0") %></td>
		  <td><%=rs.getInt("t.rtimes") %></td>
		</tr>
		<%
	}
	%>
