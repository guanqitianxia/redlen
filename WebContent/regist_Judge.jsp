...
try
{
	...
	if(usertype.equals("reader"))
	{
		...
		sql="insert into novelreaderinfo(rid,rname,rgrade,rsex,rmail,rposition,raddress,imagepath) value(?,?,?,?,?,?,?,?)";
		...
		String path_temp="reader_image/"+array[1]+".jpg";
		ps.setString(8,path_temp);
	...
	if(usertype.equals("author"))
	{
		...
		sql="insert into novelauthorinfo(aid,arealname,apenname,agrade,aidentityID,asex,apositionC,apositionX,aphone,amail,aqq,aimagepath) value(?,?,?,?,?,?,?,?,?,?,?,?)";
		ps=conn.prepareStatement(sql);
		for(int i=1;i<12;i++)
		{
			if(i==1||i==6||i==11){
		...
		String path_temp="author_image/"+array[1]+".jpg";
		ps.setString(12,path_temp);
		ps.executeUpdate();ps.close();
	}
