...
<div class="entity_search">
    ...
    <div class="entity_search_title">查询</div>
     <ul>
        ...
        <li>
        <form action="EntiSeaServlet?searchtype=bid&id=<%=id0%>&usertype=<%=usertype%>" name="entiseaID" method="post">
        <input type="number" placeholder="按书籍编号查找" id="bookID" name="bookID">
        </form>
        <img src="PIC\search.png" onclick="EntiSeaID()"/></li>
        
        <li>
        <form action="EntiSeaServlet?searchtype=bname&id=<%=id0%>&usertype=<%=usertype%>" name="entiseaName" method="post">
        <input type="text" placeholder="按书籍名称查找" id="bookName" name="bookName">
        </form>
        <img src="PIC\search.png" onclick="EntiSeaName()"/></li>

        <li>
        <form action="EntiSeaServlet?searchtype=btype&id=<%=id0%>&usertype=<%=usertype%>" name="entiseaType" method="post">
        <input type="text" placeholder="按书籍类型查找(输入拼音)" style="ime-mode:disabled" id="bookType" name="bookType">
        </form>
        <img src="PIC\search.png" onclick="EntiSeaType()"/></li>
...
try{
	...
	while(rs.next())
	{
		...
		<tr>
          ...
          <td>
          <form class="insertform" action="cartServlet?id=<%=id0%>&sql_operate=insert&bid=<%=bid %>&aid=<%=aid %>&usertype=<%=usertype%>" method="post">
