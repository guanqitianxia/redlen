...
if(bookstyle.equals("entitymanage"))
{
   ...
   <div class="entity_search">
    <div class="entity_search_title">查询</div>
     <ul>
        <form action="EntiSeaServlet?searchtype=bid&id=<%=aid%>&usertype=1" name="entiseaID" method="post">
        <li><input type="number" placeholder="按书籍编号查找" id="bookID" name="bookID"><img src="PIC\search.png" onclick="EntiSeaID()"/></li>
        </form>
        <form action="EntiSeaServlet?searchtype=bname&id=<%=aid%>&usertype=1" name="entiseaName" method="post">
        <li><input type="text" placeholder="按书籍名称查找" id="bookName" name="bookName"><img src="PIC\search.png" onclick="EntiSeaName()"/></li>
        </form>
        <form action="EntiSeaServlet?searchtype=btype&id=<%=aid%>&usertype=1" name="entiseaType" method="post">
    ...
    <div class="net_book_table">
      <table>
        ...
        <%
    		...
    		ResultSet rs=stmt.executeQuery(sql);
    		String imagepath="",name="",type0="",Llabel="",npres="";
    		String author="";
    		int pass=0;
    	...
    	<tr>
          ...
          <%
          switch(pass){
    	  case 0:out.print("未发布");break;
    	  case 1:%><span style="color:green;">成功发布</span><%
    		  break;
    	  case 2:%><span style="color:#FFCC00;">待审核</span><%
        	  break;
    	  case 3:%><span style="color:red;" data-toggle="tooltip" data-placement="left" title="禁止原因：<%=npres%>">被禁止</span><%
    		  break;
    	  default:break;
          }
          %>
